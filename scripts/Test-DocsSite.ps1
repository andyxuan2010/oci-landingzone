...
$index = Get-Content (Join-Path $outputRoot 'index.html') -Raw; if ($index -notmatch '<title>Home · OCI Landing Zone</title>') { throw 'Generated index has an unexpected title' }; if ($index -match '\{\{[A-Z_]+\}\}') { throw 'Generated index contains an unresolved template token' }
Write-Host "OCI documentation site validation passed for $($documents.Count) Markdown documents."
